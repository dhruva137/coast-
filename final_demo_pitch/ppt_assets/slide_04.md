...
- **Measured:** even with a **perfect** yaw sensor, free DR still fails **55%** of 60 s segments *(84/186 pass)*
  → the problem is **not** a better gyro
- **Our answer:** put the **road map inside the filter loop** — state lives *on* the road graph; lateral error cannot grow without bound
