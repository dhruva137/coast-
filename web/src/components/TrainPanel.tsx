...
export function TrainPanel() {
  ...
  const [label, setLabel] = useState("python -m lab.demo · real GPU/CPU train, not a fake curve");
  ...
  return (
    <div className="train-panel">
      ...
      <div className="train-kpis">
        ...
        <div>
          <span>loss</span>
          <b>{last ? last.loss.toFixed(4) : "—"}</b>
        </div>
        <div>
          <span>hold RMSE</span>
          <b>{last?.rmse != null ? `${last.rmse.toFixed(3)} m/s` : rmse ? `${rmse.model.toFixed(3)}` : "—"}</b>
        </div>
      </div>
      <LossChart ref={chartRef} epochs={epochs} />
